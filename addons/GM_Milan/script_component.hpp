#define COMPONENT GM_Milan
#define COMPONENT_BEAUTIFIED GM Milan
#include "\z\PzGrenBtl402\addons\Main\script_mod.hpp"

// #define DEBUG_MODE_FULL
// #define DISABLE_COMPILE_CACHE
// #define CBA_DEBUG_SYNCHRONOUS

#ifdef DEBUG_ENABLED_GM_MILAN
    #define DEBUG_MODE_FULL
#endif

#ifdef DEBUG_SETTINGS_GM_MILAN
    #define DEBUG_SETTINGS DEBUG_SETTINGS_GM_MILAN
#endif

#include "\z\PzGrenBtl402\addons\Main\script_macros.hpp"

#define MILAN_MAGAZINE "gm_1Rnd_milan_heat_dm92"
#define ACTION_DISTANCE 2
